...
from utils.helpers import save_to_json, save_to_csv, validate_url
...
def main():
    ...
    try:
        ...
        # Save data
        filename = f"{args.platform}_{args.query}_{args.location}"
        ...
        if args.visualize and jobs:
            visualizer = JobVisualizer(jobs)
            visualizer.plot_jobs_by_company()
            visualizer.plot_jobs_by_location()
            visualizer.plot_job_types()
